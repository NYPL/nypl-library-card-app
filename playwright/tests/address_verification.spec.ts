...
test("should display the correct headers", async ({ page }) => {
  ...
  await expect(addressVerificationPage.mainHeader).toHaveText(
    "Apply for a Library Card Online"
  );
  await expect(addressVerificationPage.subHeader).toHaveText(
    "Step 3 of 5: Address Verification"
  );
  await expect(addressVerificationPage.homeAddressHeader).toHaveText(
    "Home Address"
  );
});
