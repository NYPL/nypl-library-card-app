...
const defineDisplayElementsTests = (getAddressPage: () => AddressPage) => {
  test.describe("displays elements", () => {
    test("displays headings, link, and buttons", async () => {
      const addressPage = getAddressPage();
...
const defineEnterHomeAddressTests = (getAddressPage: () => AddressPage) => {
...
const defineErrorMessageTests = (getAddressPage: () => AddressPage) => {
...
for (const { lang, name } of SUPPORTED_LANGUAGES) {
  test.describe(`home address page in ${name} (${lang})`, () => {
    ...
    const getAddressPage = () => addressPage;

    defineDisplayElementsTests(getAddressPage);
    defineEnterHomeAddressTests(getAddressPage);
    defineErrorMessageTests(getAddressPage);
  });
}
