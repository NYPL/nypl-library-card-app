...
import { mockCreatePatronApi, mockUsernameApi } from "../utils/mock-api";
...
test.describe("edits patron information on review page", () => {
  ...
  test("enters updated personal info", async ({ page }) => {
    const pageManager = new PageManager(page);

    await test.step("enters personal info", async () => {
    ...
    await test.step("skips alternate address", async () => {
      ...
      await test.step("edits personal info on review page", async () => {
        ...
        await pageManager.reviewPage.receiveInfoCheckbox.click(); // unable to check() but unchecks
      });
      ...
      await test.step("submits mock application", async () => {
        const editedFullName = `${TEST_EDITED_PATRON.firstName} ${TEST_EDITED_PATRON.lastName}`;
        await mockCreatePatronApi(page, editedFullName, TEST_BARCODE_NUMBER);
      ...
      await test.step("displays edited name on congrats page", async () => {
        ...
        await expect(
          pageManager.congratsPage.page.getByText(editedFullName)
        ).toBeVisible();
      });
    });
